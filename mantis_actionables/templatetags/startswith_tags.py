...
def do_count(parser, token):
    try:
        ...
        tag_name, string, start_string, num = token.split_contents()
    except ValueError:
    ...
    return IfCountNode(string, start_string, nodelist_true, nodelist_false, num)


class IfCountNode(Node):
    def __init__(self, string, start_string, nodelist_true, nodelist_false, num):
    ...
    def render(self, context):
        ...
        if string.count(start_string)>=int(self.num):
...
#@register.tag
def ifnotcount(parser, token):
    return do_count(parser, token)
ifnotcount = register.tag(ifnotcount)
